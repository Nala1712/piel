...
class PhaseBitDataFrame(BaseModel):
...
PhaseBitDataFrame(bits=[], phase=[]).dict()
...
TypeAdapter(User(name="Fred", id=3))
...
def validate_phase_bit_dataframe(
    ...
    """
    Validate that the DataFrame contains 'bits' and 'phase' columns.
    """
    required_columns = {"bits", "phase"}
    if not required_columns.issubset(v.columns):
        missing_columns = required_columns - set(v.columns)
        raise ValueError(f"Missing required columns: {', '.join(missing_columns)}")

    # If the DataFrame is valid, process it with the handler
    return handler(v)


# Define a new type with the validation attached
PhaseBitDataFrame = Annotated[pd.DataFrame, WrapValidator(validate_phase_bit_dataframe)]
PhaseBitDataFrameValidate = TypeAdapter(
    PhaseBitDataFrame,
...
df_valid = {"bits": [1, 0, 1], "phase": [0, 1, 0]}

# Create a dataframe with missing columns
df_invalid = {"bits": [1, 0, 1], "amplitude": [0, 1, 0]}

validated_df_valid = PhaseBitDataFrameValidate(df_valid)
validated_df_invalid = PhaseBitDataFrameValidate(df_invalid)
# -

help(PhaseBitDataFrameValidate)

validated_df_invalid
